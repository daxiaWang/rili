(function() {
    ...
    var dateClaaArr = ['lawday', 'tradeday', 'festivalday', 'workday']
    var _viewer_ = this;
        ...
        if (config === 'setDisabledDay' || config === 'setYear' || config === 'getSelected' || config === 'acceptChange' || config === 'setColors' || config === 'setLabel' || config === 'setLawday' || config === 'initDate') { //方法
            ...
            if (config == 'setYear') { //重置年份
                    ...
                    for (var i = 0; i < param.dt.length; i++)
                        if (d == param.dt[i].d) {
                            if ($(this).hasClass("weekend")) {
                                $(this).removeClass()
                                $(this).addClass("selected weekend")
                            } else {
                                $(this).removeClass()
                ...
                if (param.type == 'remove') {
                    // console.log("me.find('table tr').find('td').eq(param.date)", me.find('table tr').find('td').eq(param.date))
                    me.find('table tr').find('td').eq(param.date).removeClass().addClass("weekend");
                    // .addClass("selected weekend")
                    // me.find('td.weekend').eq(param.date).removeClass().addClass("weekend");
                    // me.find('table tr').find('td').eq(param.date).css('background-color', 'red')
                } else {
                    if (param.date) {
                ...
                if (param) {
                    me.find('table tr:gt(1)').find('td').each(function() {
                        console.log("this", this, param, this.cellIndex)
